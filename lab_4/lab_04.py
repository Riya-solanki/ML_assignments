...
# A6: Project data with PCA
pca = PCA(n_components=2)
X_2d_project = pca.fit_transform(X)
y_2d_project = y_encoded
plt.figure(figsize=(6, 6))
plt.scatter(X_2d_project[y_2d_project == le.transform(['high'])[0]][:, 0], X_2d_project[y_2d_project == le.transform(['high'])[0]][:, 1], c='blue', label='High', alpha=0.6)
plt.scatter(X_2d_project[y_2d_project == le.transform(['medium'])[0]][:, 0], X_2d_project[y_2d_project == le.transform(['medium'])[0]][:, 1], c='red', label='Medium', alpha=0.6)
plt.xlabel('PCA Component 1')
plt.ylabel('PCA Component 2')
plt.title('Project Data: 2D PCA (High vs Medium)')
plt.legend()
plt.grid(True)
plt.show()

x_range = np.arange(X_2d_project[:, 0].min(), X_2d_project[:, 0].max(), 0.1)
y_range = np.arange(X_2d_project[:, 1].min(), X_2d_project[:, 1].max(), 0.1)
X_test_2d_project = np.array(np.meshgrid(x_range, y_range)).T.reshape(-1, 2)
knn_project = KNeighborsClassifier(n_neighbors=3)
knn_project.fit(X_2d_project, y_2d_project)
y_test_2d_project_pred = knn_project.predict(X_test_2d_project)
plt.figure(figsize=(6, 6))
plt.scatter(X_test_2d_project[y_test_2d_project_pred == le.transform(['high'])[0]][:, 0], X_test_2d_project[y_test_2d_project_pred == le.transform(['high'])[0]][:, 1], c='blue', label='Predicted High', alpha=0.1, s=10)
plt.scatter(X_test_2d_project[y_test_2d_project_pred == le.transform(['medium'])[0]][:, 0], X_test_2d_project[y_test_2d_project_pred == le.transform(['medium'])[0]][:, 1], c='red', label='Predicted Medium', alpha=0.1, s=10)
plt.scatter(X_2d_project[y_2d_project == le.transform(['high'])[0]][:, 0], X_2d_project[y_2d_project == le.transform(['high'])[0]][:, 1], c='blue', edgecolors='black', label='Training High', s=100)
plt.scatter(X_2d_project[y_2d_project == le.transform(['medium'])[0]][:, 0], X_2d_project[y_2d_project == le.transform(['medium'])[0]][:, 1], c='red', edgecolors='black', label='Training Medium', s=100)
plt.xlabel('PCA Component 1')
plt.ylabel('PCA Component 2')
plt.title('Project Test Data Predictions (k=3)')
plt.legend()
plt.grid(True)
plt.show()
...
for k in k_values:
    ...
    plt.xlabel('PCA Component 1')
    plt.ylabel('PCA Component 2')
    plt.title(f'Project Test Data Predictions (k={k})')
    plt.legend()
    plt.grid(True)
    plt.show()
